...
def gather_friends(api, ids, following_limit = 10000, file_name=None) :
    ...
    for this_id in ids :
        print("Pulling following for " + str(this_id))        
        pulled_following = 0
        try :
            for page in tweepy.Cursor(api.friends_ids,user_id = this_id).pages() :
                pulled_following += len(page['ids'])
                ret_dict[this_id].extend(page['ids'])
                print("Number pulled: " + str(pulled_following))                
                
                if following_limit and pulled_following > following_limit :
                    break
...
def gather_following(api, ids, following_limit = 10000, file_name=None) :
    ''' from a set of IDs, get all the people they follow's ids up to follower
        limit. Returns a dictionary with key = id, value = following_id list
    '''
    ...
    for this_id in ids :
        ...
        try :
            for page in tweepy.Cursor(api.friends_ids,user_id = this_id).pages() :
                pulled_following += len(page['ids'])
                ret_dict[this_id].extend(page['ids'])
                print("Number pulled: " + str(pulled_followeing))                
